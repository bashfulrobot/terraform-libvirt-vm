...
resource "libvirt_volume" "addtional-volume-qcow2" {
  count            = var.vm_count
  name             = format("${var.vm_hostname_prefix}-data-%02d.qcow2", count.index + var.index_start)
  pool             = var.pool
  size             = 1024 * 1024 * 1024 * var.data_volume
  base_volume_id   = var.base_volume_name != null ? null : element(libvirt_volume.base-volume-qcow2, 0).id
  base_volume_name = var.base_volume_name
  base_volume_pool = var.base_pool_name
